...
'''
autos.plot(kind='scatter', x='weight', y='mpg') #Pretty clear from the scatter chart
autos.plot(kind='scatter', x='displacement', y='horsepower') #Correlation equals causation
autos.acceleration.hist()
autos.mpg.hist(by=autos.cylinders, sharex=True, sharey=True)
autos['pre1975'] = np.where(autos.model_year > 75, 1, 0)
autos.groupby('pre1975').mpg.mean().plot(kind='bar')
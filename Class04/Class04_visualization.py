...
#drinks = pd.read_table('https://raw.githubusercontent.com/justmarkham/DAT5/master/data/drinks.csv', sep=',')
...
drinks.beer_servings.hist(by=drinks.continent) #The hist plot has a grouping by variable
plt.show()

#We can specify that each chart use the same scale for an axis
...
drinks.beer_servings.hist(by=drinks.continent, sharex=True, sharey=True, layout=(2,3))
plt.show()
...
drinks.plot(kind='scatter', x='beer_servings', y='wine_servings', alpha=.2)
plt.show()
...
drinks.plot(kind='scatter', x='spirit_servings', y='total_litres_of_pure_alcohol', alpha=0.33)
plt.show()

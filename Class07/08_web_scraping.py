...
fav = b.findAll(name='li', attrs={'id':'my favorite'})
print fav
...
infobars = b.find(name='div', attrs={'class':'infobar'})
...
#content_rating = infobar.find(name='meta', attrs={'itemprop':'contentRating'}).text
#Why does the above line return a null unicode object?  Why must ['content'] be used?

# Retrieve the show's duration
duration = infobar.find(name='time', attrs={"itemprop":"duration"}).text
#OK, so ['content'] is used to get the content that the browser would render and text should be used when?
...
#1) Find the div with all of the actors listed and extract it
actors_raw = b.find(name='div', attrs={'class':'txt-block', 'itemprop':'actors', 'itemscope':'', 'itemtype':'http://schema.org/Person'})

#2) Extract each actor's name
actors = actors_raw.findAll(name='span', attrs={'itemprop':'name'})

3) Loop through the actors ResultSet and create a list containing each actor name
